...
class RatingController extends Controller {
	...
	public function getRecensyRating($shop_id=1, $user_id=1){// trả về hạng Recensy (R1,R2,R3,R4) cho user tương ứng với 1 shop
	...
	public function getFrequencyRating($shop_id=1, $user_id=1){// trả về hạng Frequency (F1,F2,F3,F4) cho user tương ứng với 1 shop
	...
	public function getMonetaryRating($shop_id=1, $user_id=1){// trả về hạng Monetary (M1,M2,M3,M4) cho user tương ứng với 1 shop
		$data = $this->order_model->getCountMonetary($shop_id, $user_id, $this->monetary_day);
		return $this->getMonetary($data['price']);
	}
	
	public function getRatingUser($shop_id=1, $user_id=1){
		$rating = 0;//giá trị mặc định
		$recensy = $this->getRecensyRating($shop_id, $user_id);
		$frequency = $this->getFrequencyRating($shop_id, $user_id);
		$monetary = $this->getMonetaryRating($shop_id, $user_id);
	...
	// anhth test push noti
	public function getPushNoti(){
		$arr_token = array("fJeegrmUJ6Q:APA91bGk9sSYo4eEz9h0QBkIBIX-yJhsYEB08YxmbPEk5TAs_6rKA3a1YYVkXY0rnjwXSNwpQZ7rqS0kp-S-mktQfiBg2LNIldnPRB5K7svn0FwkyimEzpjt41V5Tj8J-_eju5KfVYnD");
		$msg = array
		(
		...
		var_dump($this->sendNotification($arr_token,$msg)); 
	}
}
